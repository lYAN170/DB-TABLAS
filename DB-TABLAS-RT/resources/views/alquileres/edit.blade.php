        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <form action="{{ route('alquileres.update', $alquiler->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="inquilinos[]" class="form-control" multiple required>
                            @foreach($inquilinos as $inquilino)
                                <option value="{{ $inquilino->id }}" {{ in_array($inquilino->id, $alquiler->inquilinos->pluck('id')->toArray()) ? 'selected' : '' }}>
                                    {{ $inquilino->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="departamento_id" class="form-control" required>
                            @foreach($departamentos as $departamento)
                                <option value="{{ $departamento->id }}" {{ $alquiler->departamento_id == $departamento->id ? 'selected' : '' }}>
                    ...
                    <a href="{{ route('alquileres.index') }}" class="btn btn-success mt-7">{{ __('Volver') }}</a>
                </form>
            </div>
        </div>
    </div>
